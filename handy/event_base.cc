...
struct EventsImp {
    ...
    void wakeup() {
        ...
        handy_fatalif(r <= 0, "write handy_error wd %d %d %s", r, errno, strerror(errno));
    }
...
void EventsImp::init() {
    ...
    handy_fatalif(r, "pipe failed %d %s", errno, strerror(errno));
    r = util::addFdFlag(wakeupFds_[0], FD_CLOEXEC);
    handy_fatalif(r, "addFdFlag failed %d %s", errno, strerror(errno));
    r = util::addFdFlag(wakeupFds_[1], FD_CLOEXEC);
    handy_fatalif(r, "addFdFlag failed %d %s", errno, strerror(errno));
    handy_trace("wakeup pipe created %d %d", wakeupFds_[0], wakeupFds_[1]);
    ...
    ch->onRead([=] {
        ...
        if (r > 0) {
            ...
            handy_fatal("wakeup channel read handy_error %d %d %s", r, errno, strerror(errno));
        }
    });
}
...
Channel::Channel(EventBase *base, int fd, int events) : base_(base), fd_(fd), events_(events) {
    handy_fatalif(net::setNonBlock(fd_) < 0, "channel set non block failed");
...
void Channel::close() {
    if (fd_ >= 0) {
        handy_trace("close channel %ld fd %d", (long) id_, fd_);
...
TcpConn::~TcpConn() {
    handy_trace("tcp destroyed %s - %s", local_.toString().c_str(), peer_.toString().c_str());
    delete channel_;
}
...
void TcpConn::reconnect() {
    ...
    handy_info("reconnect interval: %d will reconnect after %lld ms", reconnectInterval_, interval);
