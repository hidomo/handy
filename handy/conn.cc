...
void TcpConn::attach(EventBase *base, int fd, Ip4Addr local, Ip4Addr peer) {
    handy_fatalif((destPort_ <= 0 && state_ != State::Invalid) || (destPort_ >= 0 && state_ != State::Handshaking),
    ...
    handy_trace("tcp constructed %s - %s fd: %d", local_.toString().c_str(), peer_.toString().c_str(), fd);
...
void TcpConn::connect(EventBase *base, const string &host, unsigned short port, int timeout, const string &localip) {
    handy_fatalif(state_ != State::Invalid && state_ != State::Closed && state_ != State::Failed, "current state is bad state to connect. state: %d", state_);
    destHost_ = host;
    destPort_ = port;
    ...
    handy_fatalif(fd < 0, "socket failed %d %s", errno, strerror(errno));
    net::setNonBlock(fd);
    int t = util::addFdFlag(fd, FD_CLOEXEC);
    handy_fatalif(t, "addFdFlag FD_CLOEXEC failed %d %s", t, strerror(t));
    int r = 0;
    if (localip.size()) {
        Ip4Addr addr(localip, 0);
        r = ::bind(fd, (struct sockaddr *) &addr.getAddr(), sizeof(struct sockaddr));
        handy_error("bind to %s failed handy_error %d %s", addr.toString().c_str(), errno, strerror(errno));
    }
    if (r == 0) {
        r = ::connect(fd, (sockaddr *) &addr.getAddr(), sizeof(sockaddr_in));
        if (r != 0 && errno != EINPROGRESS) {
            handy_error("connect to %s handy_error %d %s", addr.toString().c_str(), errno, strerror(errno));
        }
    }
    ...
    if (r == 0) {
        ...
        if (r < 0) {
            handy_error("getsockname failed %d %s", errno, strerror(errno));
        }
    }
    state_ = State::Handshaking;
...
void TcpConn::cleanup(const TcpConnPtr &con) {
    ...
    handy_trace("tcp closing %s - %s fd %d %d", local_.toString().c_str(), peer_.toString().c_str(), channel_ ? channel_->fd() : -1, errno);
...
void TcpConn::handleRead(const TcpConnPtr &con) {
    ...
    while (state_ == State::Connected) {
        ...
        if (channel_->fd() >= 0) {
            ...
            handy_trace("channel %lld fd %d readed %d bytes", (long long) channel_->id(), channel_->fd(), rd);
        }
        if (rd == -1 && errno == EINTR) {
            continue;
...
int TcpConn::handleHandshake(const TcpConnPtr &con) {
    handy_fatalif(state_ != Handshaking, "handleHandshaking called when state_=%d", state_);
    struct pollfd pfd;
    pfd.fd = channel_->fd();
    ...
    if (r == 1 && pfd.revents == POLLOUT) {
        ...
        if (state_ == State::Connected) {
            connectedTime_ = util::timeMilli();
            handy_trace("tcp connected %s - %s fd %d", local_.toString().c_str(), peer_.toString().c_str(), channel_->fd());
            if (statecb_) {
                statecb_(con);
            }
        }
    } else {
        handy_trace("poll fd %d return %d revents %d", channel_->fd(), r, pfd.revents);
        cleanup(con);
        return -1;
    }
    return 0;
...
void TcpConn::handleWrite(const TcpConnPtr &con) {
    if (state_ == State::Handshaking) {
        ...
        handy_error("handle write unexpected");
    }
}
...
ssize_t TcpConn::isend(const char *buf, size_t len) {
    ...
    while (len > sended) {
        ...
        handy_trace("channel %lld fd %d write %ld bytes", (long long) channel_->id(), channel_->fd(), wd);
        if (wd > 0) {
            sended += wd;
            continue;
            ...
            handy_error("write handy_error: channel %lld fd %d wd %ld %d %s", (long long) channel_->id(), channel_->fd(), wd, errno, strerror(errno));
            break;
        }
    }
    return sended;
...
void TcpConn::send(Buffer &buf) {
    if (channel_) {
        ...
        handy_warn("connection %s - %s closed, but still writing %lu bytes", local_.toString().c_str(), peer_.toString().c_str(), buf.size());
    }
}
...
void TcpConn::send(const char *buf, size_t len) {
    if (channel_) {
        ...
        handy_warn("connection %s - %s closed, but still writing %lu bytes", local_.toString().c_str(), peer_.toString().c_str(), len);
    }
}
    ...
    onRead([cb](const TcpConnPtr &con) {
        ...
        while (r) {
            ...
            if (r < 0) {
                ...
                handy_trace("a msg decoded. origin len %d msg len %ld", r, msg.size());
                cb(con, msg);
...
int TcpServer::bind(const std::string &host, unsigned short port, bool reusePort) {
    ...
    handy_fatalif(r, "set socket reuse option failed");
    r = net::setReusePort(fd, reusePort);
    handy_fatalif(r, "set socket reuse port option failed");
    r = util::addFdFlag(fd, FD_CLOEXEC);
    handy_fatalif(r, "addFdFlag FD_CLOEXEC failed");
    r = ::bind(fd, (struct sockaddr *) &addr_.getAddr(), sizeof(struct sockaddr));
    if (r) {
        close(fd);
        handy_error("bind to %s failed %d %s", addr_.toString().c_str(), errno, strerror(errno));
        return errno;
    }
    r = listen(fd, 20);
    handy_fatalif(r, "listen failed %d %s", errno, strerror(errno));
    handy_info("fd %d listening at %s", fd, addr_.toString().c_str());
...
TcpServerPtr TcpServer::startServer(EventBases *bases, const std::string &host, unsigned short port, bool reusePort) {
    ...
    if (r) {
        handy_error("bind to %s:%d failed %d %s", host.c_str(), port, errno, strerror(errno));
    }
    return r == 0 ? p : NULL;
}
...
void TcpServer::handleAccept() {
    ...
    while (lfd >= 0 && (cfd = accept(lfd, (struct sockaddr *) &raddr, &rsz)) >= 0) {
        ...
        if (r < 0) {
            handy_error("get peer name failed %d %s", errno, strerror(errno));
            continue;
        }
    ...
    if (lfd >= 0 && errno != EAGAIN && errno != EINTR) {
        handy_warn("accept return %d  %d %s", cfd, errno, strerror(errno));
    }
}
