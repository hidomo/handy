...
int UdpServer::bind(const std::string &host, unsigned short port, bool reusePort) {
    ...
    net::setNonBlock(fd);
    handy_info("udp fd %d bind to %s", fd, addr_.toString().c_str());
    ...
    channel_->onRead([this] {
        ...
        if (rn < 0) {
            handy_error("udp %d recv failed: %d %s", fd, errno, strerror(errno));
            return;
        }
        buf.addSize(rn);
        handy_trace("udp %d recv %ld bytes from %s", fd, rn, Ip4Addr(raddr).toString().data());
...
void UdpServer::sendTo(const char *buf, size_t len, Ip4Addr addr) {
    if (!channel_ || channel_->fd() < 0) {
        handy_warn("udp sending %lu bytes to %s after channel closed", len, addr.toString().data());
        return;
    }
    int fd = channel_->fd();
    int wn = ::sendto(fd, buf, len, 0, (sockaddr *) &addr.getAddr(), sizeof(sockaddr));
    if (wn < 0) {
        handy_error("udp %d sendto %s handy_error: %d %s", fd, addr.toString().c_str(), errno, strerror(errno));
        return;
    }
    handy_trace("udp %d sendto %s %d bytes", fd, addr.toString().c_str(), wn);
}
...
UdpConnPtr UdpConn::createConnection(EventBase *base, const string &host, unsigned short port) {
    ...
    handy_trace("udp fd %d connecting to %s ok", fd, addr.toString().data());
    ...
    ch->onRead([con] {
        ...
        if (rn < 0) {
            handy_error("udp read from %d handy_error %d %s", fd, errno, strerror(errno));
            return;
        }
        handy_trace("udp %d read %d bytes", fd, rn);
        input.addSize(rn);
        con->cb_(con, input);
    });
    return con;
...
void UdpConn::send(const char *buf, size_t len) {
    if (!channel_ || channel_->fd() < 0) {
        handy_warn("udp sending %lu bytes to %s after channel closed", len, peer_.toString().data());
        return;
    }
    int fd = channel_->fd();
    int wn = ::write(fd, buf, len);
    if (wn < 0) {
        handy_error("udp %d write handy_error %d %s", fd, errno, strerror(errno));
        return;
    }
    handy_trace("udp %d write %d bytes", fd, wn);
}
