...
void HttpConnPtr::handleRead(const HttpCallBack &cb) const {
    if (!tcp->isClient()) {  // server
        ...
        if (r == HttpMsg::Continue100) {
            ...
            handy_info("http request: %s %s %s", req.method.c_str(), req.query_uri.c_str(), req.version.c_str());
            handy_trace("http request:\n%.*s", (int) tcp->input_.size(), tcp->input_.data());
            cb(*this);
        }
    } else {
        HttpResponse &resp = getResponse();
        ...
        if (r == HttpMsg::Complete) {
            handy_info("http response: %d %s", resp.status, resp.statusWord.c_str());
            handy_trace("http response:\n%.*s", (int) tcp->input_.size(), tcp->input_.data());
            cb(tcp);
        }
    }
}
